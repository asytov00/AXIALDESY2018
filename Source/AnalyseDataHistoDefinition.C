...
TH2F *hxy2P = new TH2F("hxy2P","Y Vs X for detector 2;X  [#mum];Y  [#mum]",detectXBin,detectX1,detectX2,detectYBin,detectY1,detectY2);
...
//thXin+0.5,thXin+0.5,thXinBin,-thXin+0.5,thXin+0.5
TH2F *hthXYinP = new TH2F("hthXYinP","In-going divergence (#theta_{x,in},#theta_{y,in});Horizontal Incoming Angle [#murad];Vertical Incoming Angle [#murad]",thXinBin,-thXin+4.,thXin+4.,thXinBin,-thXin+4.,thXin+4.);
...
TH2F *CradthYin2018Ph_gamma = new TH2F("CradthYin2018Ph_gamma","Gamma: Calorimeter 4 Vs Cradle angle + Y in-going angle (#theta_{x,in}) ; Horizontal Incoming Angle [#murad]; Energy [GeV]; Counts [a.u.]",(nbin+4)*2*10,rotSX,rotDX,binCalo,EminCalo,EmaxCalo);
...
//1D histograms for calorimeters: gamma and particle energy
TH1F *hgamma = new TH1F("hgamma", "Gamma calorimemeter;Energy [GeV]",binCalo,EminCalo,EmaxCalo);
...
TH1F *ScinP= new TH1F("ScinP","Scintillator; ADC counts;   ",400,-200,19800);
//Scintillator signal
...
TH1F *hPhotomP= new TH1F("hPhotomP", "Photomultiplier; ADC counts", 400,-200,19800);


TH2F *hthXinPhotomP= new TH2F("hthXinPhotomP","Photomultiplier VS X in-going angle (#theta_{x,in}); Horizontal Incoming Angle [#murad]; ADC counts",thXinBin,-thXin*2+0.5,thXin*2+0.5,400,-200,19800);
TH2F *hthYinPhotomP= new TH2F("hthYinPhotomP","Photomultiplier VS Y in-going angle (#theta_{y,in}); Vertical Incoming Angle [#murad]; ADC counts",thXinBin,-thYin*2+0.5,thYin*2+0.5,400,-200,19800);
...
TH2F *hRotPhotomP = new TH2F("hRotPhotomP","Photomultiplier Vs Rotational; Rotational Angle [#murad]; ADC counts",Nsteps_rot,gonio_rot_first-0.5*binSizeRot,gonio_rot_last-0.5*binSizeRot,100,0,20000);

TH2F *hRotPhotom_pinP = new TH2F("hRotPhotom_pinP", "Photomultiplier Vs Rotational + X in-going angle (#theta_{x,in}); Scan Angle [#murad]; ADC counts",(nbin+4)*20,rotSX,rotDX,100,0,20000);

TH2F *hRotPhotom_minP = new TH2F("hRotPhotom_minP","Photomultiplier Vs Rotational - X in-going angle (#theta_{x,in}); Scan Angle [#murad]; ADC counts",(nbin+4)*20,rotSX,rotDX,100,0,20000);


TH2F *hCradPhotomP = new TH2F("hCradPhotomP","Photomultiplier Vs Cradle Scan; Cradle Angle [#murad]; ADC counts",Nsteps_crad,rotSX,rotDX+1000,100,0,20000);
//TH2F *CradScinP = new TH2F("CradScinP","Scintillator Vs Cradle Scan; Cradle Angle [#murad]; Scintillator signal",Nsteps_crad,gonio_crad_first-0.5*binSizeCrad,gonio_crad_last-0.5*binSizeCrad,400,-200,19800);

TH2F *hCradPhotom_pinP = new TH2F("hCradPhotom_pinP", "Photomultiplier Vs Cradle + Y in-going angle (#theta_{y,in}); Scan Angle [#murad]; ADC counts",(nbin+4)*20,rotSX,rotDX,100,0,20000);

TH2F *hCradPhotom_minP = new TH2F("hCradPhotom_minP","Photomultiplier Vs Cradle - Y in-going angle (#theta_{y,in}); Scan Angle [#murad]; ADC counts",(nbin+4)*20,rotSX,rotDX,100,0,20000);
